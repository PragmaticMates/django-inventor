...
for listing_type in Listing.__subclasses__():
    url_name = f'{listing_type.__name__.lower()}_list'
    verbose_name_plural = ugettext_lazy(listing_type._meta.verbose_name_plural)
    url_path = slugify(verbose_name_plural) + '/'
    urlpatterns.append(path(url_path, ListingListView.as_view(template_name='listings/listing_list.html', model=listing_type), name=url_name))  # TODO: move to Listing class
